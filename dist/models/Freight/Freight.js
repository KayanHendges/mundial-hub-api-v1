...
class Freight {
    async freteTray(params, res) {
        const date = (0, date_fns_1.getTime)((0, date_fns_1.addHours)(new Date(), -3));
        ...
        axios_1.default.get(query).then(response => {
            const newDate = (0, date_fns_1.getTime)((0, date_fns_1.addHours)(new Date(), -3));
            console.log('tempo de resposta Frenet', newDate - date);
            const json = xml_js_1.default.xml2js(response.data, {
                textKey: "text",
                compact: true
            });
        ...
        async function getResponseTray(params, json, res) {
            ...
            function responseJaraguaDoSul(json, res) {
                const resultados = [
                    {
                        codigo: { text: '441' },
                ...
                const xml = xml_js_1.default.json2xml(json, {
                    compact: true,
                    ignoreComment: true,
                    textKey: "text",
                    spaces: 0
            ...
            async function responseBrasil(params, json, res) {
                ...
                if (cheapDelivery.codigo.text == expressDelivery.codigo.text ||
                ...
                const endDate = (0, date_fns_1.getTime)((0, date_fns_1.addHours)(new Date(), -3));
                console.log('==== fim', endDate - date, '===============');
                res.send(xml);
            }
            async function validateShippingList(shippingList) {
                const list = [];
                const postDay = 1;
            ...
            function deliveryPrice(params, price) {
            ...
            function getExpressDelivery(deliveryList) {
    ...
    async freteViaVarejo(params, res) {
        ...
        async function getDimensions(id, amount) {
            ...
            function hasPromotionalPrice(resultado) {
        ...
        function xmlToJson(xml, trayProductId) {
    ...
    async getProductTime(trayId, quantity) {
        ...
        return 4;
        async function checkKitQuantity(trayId) {
        ...
        async function getProvider(trayId, quantity) {
            return new Promise(resolve => {
