...
class Tables implements ITables {
    ...
    createProviderProducts(){
        const sql = 'CREATE TABLE IF NOT EXISTS providers_products (provider_product_id INT NOT NULL AUTO_INCREMENT, provider_id INT(9), product_reference INT(9), hub_id INT(9), product_name VARCHAR(200), product_brand VARCHAR(64), product_stock INT(9), product_price FLOAT(9), additional_costs VARCHAR(200), PRIMARY KEY(provider_product_id), FOREIGN KEY (provider_id) REFERENCES providers(provider_id))'
